package com.marcosoft.uberclone10.activities.client;
...
import com.marcosoft.uberclone10.providers.AuthProvider;
import com.marcosoft.uberclone10.providers.ClientBookingProvider;
import com.marcosoft.uberclone10.providers.ClientProvider;
import com.marcosoft.uberclone10.providers.GeofireProvider;
import com.marcosoft.uberclone10.providers.GoogleApiProvider;
import com.marcosoft.uberclone10.providers.NotificationProvider;
import com.marcosoft.uberclone10.providers.TokenProvider;
import com.marcosoft.uberclone10.utils.DecodePoints;
...
public class RequestDriverActivity extends AppCompatActivity {
    ...
    private ClientBookingProvider mClientBookinProvider;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mClientBookinProvider = new ClientBookingProvider();
        ...
        mTokenProvider.getToken(mIdDriverFound).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                *//*if (snapshot.exists()) {
                    ...
                    map.put("body", "Un cliente está solicitndo un servicio");*//*
                if (snapshot.exists()) {
                    String token = snapshot.child("token").getValue().toString();
                    Map<String, String> map = new HashMap<>();
                    map.put("title", "VIAJE SOLICITADO");
                    map.put("body",
                            "El cliente enviado la solicitud"
                    );
                    FCMBody fcmBody = new FCMBody(token, "high", "4500s", map);
                    Log.d("body", "ID: " + fcmBody.getData().toString());
                    mNotificationProvider.sendNotification(fcmBody).enqueue(new Callback<FCMResponse>() {
                        @Override
                        public void onResponse(Call<FCMResponse> call, Response<FCMResponse> response) {
                            if (response.body() != null) {
                                if (response.body().getSuccess() == 1) {
                                    ...
                                    Toast.makeText(RequestDriverActivity.this, "No se pudo enviar la notificacion error 1", Toast.LENGTH_SHORT).show();
                                }
                            }
                            else {
                                Toast.makeText(RequestDriverActivity.this, "No se pudo enviar la notificacion error 2", Toast.LENGTH_SHORT).show();
                            }
                        }
